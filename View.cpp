...
void View::draw()
{
	time += 0.001;
	sgraph.animate(time);
	sgraph.animate_cone(3*cam_x, 3*cam_y);
	sgraph.animate_flag(3*(flag_x), 3*(flag_y));
	double PI = 3.14159;
    /*
	...
	else if(cam_state==2)
	{
		if(direction==0)
		{
			modelview.top() = modelview.top() * glm::lookAt(glm::vec3(3*cam_x,3*cam_y,1),glm::vec3(cam_x,-5000,0),glm::vec3(0,0,1));
		}
		else if(direction==1)
		{
			modelview.top() = modelview.top() * glm::lookAt(glm::vec3(3*cam_x,3*cam_y,1),glm::vec3(-5000,cam_y,0),glm::vec3(0,0,1));
		}
		else if(direction==2)
		{
			modelview.top() = modelview.top() * glm::lookAt(glm::vec3(3*cam_x,3*cam_y,1),glm::vec3(cam_x,5000,0),glm::vec3(0,0,1));
